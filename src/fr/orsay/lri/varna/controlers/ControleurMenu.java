 ...
 * 
 */
public class ControleurMenu extends InterfaceVARNAListener implements
		ActionListener {
	...
	private boolean colorBases() {
		...
		if (_color.equals("InnerColor")) {
			...
			if (c != null) {
				...
				for (int i = 0; i < listBase.size(); i++) {
					...
					if (mb.getElementStructure() != -1) {
						mb.getStyleBP().setCustomColor(c);
					}
				}
				_vp.repaint();
			}
			...
			ArrayList<ModeleStyleBP> styleBPs = new ArrayList<ModeleStyleBP>();
			for (int i = 0; i < listBase.size(); i++) {
				ModeleBase mb = _vp.getRNA().get_listeBases().get(
						listBase.get(i));
				if (mb.getElementStructure() != -1) {
	...
	/**
	 * Open a warning message dialog with the message to display
	 * 
	 * @param warning
	 *            The warning <code>String</code> to display
	 */
	public void warningEmitted(String warning) {
		if (_vp.getPopupMenu().get_itemShowWarnings().getState()) {
			JOptionPane.showMessageDialog(_vp, warning, "VARNA Warning",
