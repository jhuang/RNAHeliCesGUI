...
public class VueBPThickness implements ChangeListener {
	...
	public VueBPThickness(VARNAPanel vp, ArrayList<ModeleStyleBP> msbp) {
		_vp = vp;
		_msbp = msbp;
		backupThicknesses();

		_thicknessSlider = new JSlider(JSlider.HORIZONTAL, 1, 100,
				(int) (msbp.get(0).getThickness(
		...
		JLabel thicknessLabel = new JLabel(String.valueOf(msbp.get(0)
	...
	private void backupThicknesses() {
		for (int i = 0; i < _msbp.size(); i++) {
			this._backupThicknesses.add(_msbp.get(i).getThickness(
	...
	public void restoreThicknesses() {
		for (int i = 0; i < _msbp.size(); i++) {
			_msbp.get(i).setThickness(_backupThicknesses.get(i));
		}
	}
