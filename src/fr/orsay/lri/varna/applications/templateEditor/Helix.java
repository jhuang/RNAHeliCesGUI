...
public class Helix extends GraphicalTemplateElement{
  ...
  private static int _numHel = 1;  
  
  public Helix(double x, double y, RNATemplate tmp)
  {
	  this(x,y,getNextAutomaticCaption(),tmp);
  }
  
  public Helix(double x, double y, String cap, RNATemplate tmp)
  {
	  _h = tmp.new RNATemplateHelix();
  ...
  public Helix(RNATemplateHelix templateHelix) {
	  _h = templateHelix;
	  if (! _h.hasCaption()) {
		  _h.setCaption(getNextAutomaticCaption());
	  }
  }
  
  
  private static String getNextAutomaticCaption() {
	  return "H"+_numHel++;
  }
  ...
  public double getExtentY()
  {
	...
	public static final double BASE_PAIR_DISTANCE = 30.0;
	public static final double BASE_DISTANCE = 15.0;
	...
	public Polygon getSelectionBox()
	{
		double dx = (_h.getStartPosition().x-_h.getEndPosition().x)/(_h.getStartPosition().distance(_h.getEndPosition()));
		double dy = (_h.getStartPosition().y-_h.getEndPosition().y)/(_h.getStartPosition().distance(_h.getEndPosition()));
		double nx = dy;
	...
	public int getNbBP()
	{
		...
		return (int)Math.floor(helLength/Helix.BASE_DISTANCE)+2;		  
	}
