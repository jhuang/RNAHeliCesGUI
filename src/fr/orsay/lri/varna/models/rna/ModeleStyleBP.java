...
public class ModeleStyleBP implements Serializable {
	...
	public ModeleStyleBP() {
		this(new ModeleBaseNucleotide(-1), new ModeleBaseNucleotide(-1));
	}

	public ModeleStyleBP(ModeleBase part5, ModeleBase part3) {
		this(part5, part3, Edge.WATSON_CRICK, Edge.WATSON_CRICK, Stericity.CIS);
	}

	private static Random rnd = new Random(System.currentTimeMillis());

	public ModeleStyleBP(ModeleBase part5, ModeleBase part3, Edge edge5,
		...
		_edge3 = edge3;
		_stericity = ster;
		// _edge1 = getRandomEdge();
		// _edge2 = getRandomEdge();
		// _stericity = getRandomStericity();
	}
	...
	public void assignParameters(String parametersValue)
		...
		for (int i = 0; i < namesArray.size(); i++) {
			if (namesArray.get(i).toLowerCase().equals(PARAM_COLOR)) {
				try {
					this.setCustomColor(ModeleStyleBase
				...
				try {
					this.setThickness(Double.parseDouble(valuesArray.get(i)));
				} catch (NumberFormatException e) {
					throw new ExceptionParameterError(e.getMessage(),
