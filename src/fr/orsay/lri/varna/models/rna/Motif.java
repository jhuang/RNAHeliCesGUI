/*
 ...
 paper mail : LRI, bat 490 UniversitÔøΩ Paris-Sud 91405 Orsay Cedex France
...
public class Motif implements Serializable{
	 ...
	 * @return the number of adjustment executed in the same strand 
	 */
	public void positionneSpecificStrand(int i, double d){
		...
		int part5Temp=-1;
			...
			for (int j = 0; j < getListStrand().getStrand(i).sizeStrand(); j++ ){
				...
				//On positionne la premi√®re base du brin
	...
	public void ajusteSpecificStrand(int i){					
		for (int j = 0; j < this.getListStrand().getStrand(i).sizeStrand(); j++ ){
			...
			if((!getListStrand().getStrand(i).existInStrand(partner)||!getListStrand().getStrand(i).existInStrand(indice))&&
				...
				//On change les coordonn√©es du nucleotide present sur le brin actuel
	...
	public void reajustement(){
		...
		for (int i = 0; i < getListStrand().sizeStruct(); i++ ){
			...
			int strandModified = -1; //brin sur lequel les changements de coord ont ÈtÈ effectuÈ
			...
			if(partnerLast !=-1 && partnerFirst !=-1){
				...
				if(size > getListStrand().getStrand(numStrandPartner).sizeStrand()){
					//On modifiera les coord des bases prÈsentes sur le brin le plus court
	 ...
	 * @Return the index of the strand which is the central strand in a star pattern. 
	 * @Return -1 if there is no central strand.
	 */
	...
	public void orderStrands(int centralStrand){
		...
		//htableLast : les cl√©s correspondent aux brins, les valeurs correspondent 
		//√† la derni√®re base du brin central en interaction avec un autre brin
		//htableFirst : les valeurs correspondent √† la permi√®re base du brin central
		...
		for(int j = 0; j < this.getListStrand().getStrand(centralStrand).sizeStrand(); j++ ){
			...
			// Si la base est li√©e √† un autre brin
			if (partner!=-1) {
		...
		//int comptDroite=0; //permet de savoir si un brin a √©t√© plac√© √† droite.
		//int comptGauche=0;
		...
		for (int j = 0; j < this.getListStrand().getStrand(centralStrand).sizeStrand(); j++ ){
			...
			if(indexPartenaire!=-1 && !this.getListStrand().getStrand(partenaire.getNumStrand()).hasBeenPlaced()){
				...
				//Si la base n'est pas comprise dans un intervalle √† droit et √† gauche
				...
				//Si la base est comprise dans un intervalle √† droite, on place le brin √† gauche
				...
				//Si la base est comprise dans un intervalle √† gauche, on place le brin √† droite
				else if(!baseStrand.getIntervDroite() && baseStrand.getIntervGauche()){
					getListStrand().getStrand(partenaire.getNumStrand()).setStrandRight(true);
					
				}
				
				//Cas ou la base est comprise √† la fois dans un intervalle √† gauche et √† droite
				...
				if(previousStrand!=-1 && previousStrand!=partenaire.getNumStrand()){
					...
					if(getListStrand().getStrand(previousStrand).getStrandLeft()&& 
						...
						//Si le brin prÈcÈdent et le brin actuel sont du mÍme cotÈ et au mÍme niveau (sur le mÍme axe vertical)
						System.out.println("TEST2");
						
						//le partenaire de la derniËre base du brin centrale reliÈe au brin prÈcÈdent.
						...
						//coord Y de la derniere base du brin prÈcÈdent en intÈraction avec le brin central
						double coordYLastBase= getListStrand().getStrand(centralStrand).
							getMB(htableLast.get(previousStrand)).getCoords().y;
						
						//coord Y de la premiere base du brin actuel en intÈraction avec le brin central
						...
						//et la derniËre base du brin partenaire
						...
						//distance entre la derniere base du brin prÈcÈdent en interaction avec le brin central et 
						//et la 1ere base du brin prÈcÈdent
						...
						if((distB+distC+1)> dist){
							...
							//on modifie la coordonnÈe y de la base actuelle sur le brin central
				...
				if(j!=indice){
					if ((coordY==coordYBP && coordY==coordYBPpartner) || 
						...
						if(((coordX<coordXBP && coordX>coordXBPpartner)||(coordX<coordXBPpartner && coordX>coordXBP))
							...
							if(!basesPair.get(i).getStyleBP().isBent()){
								basesPair.get(i).getStyleBP().setBent(1.0);
							}
						}
					}
				}
	...
	public ArrayList<ModeleBase> listBasePair(){
		...
		for (int i = 0; i < getListStrand().sizeStruct(); i++ ){
			...
			for (int j = 0; j < getListStrand().getStrand(i).sizeStrand(); j++ ){
				...
				// Si la base est li√©e √† une autre base
