  ...
  public static final int NUM_STEPS_ROUNDED_CORNERS = 10;
	...
	public GeneralPath getShape(Point2D.Double[] realCoords,Point2D.Double[] realCenters, double scaleFactor)
	{
		...
		if (getBases().size()>=2)
		{
			...
			isDirect = ControleurClicMovement.testDirectionality(p1, p3, p2);
		}
		if (getBases().size()>0)
		{
			int j = getBases().get(0).getIndex();
		...
		{ 
			...
			if ((j1>0)&&(j1<realCoords.length-1))
			{
				...
				double dist = p2.distance(p0);
				Point2D.Double v = new Point2D.Double((p2.x-p0.x)/dist,(p2.y-p0.y)/dist);
				Point2D.Double vn = new Point2D.Double(v.y,-v.x);

				Point2D.Double interForward = new Point2D.Double(p1.x + scaleFactor *getRadius()*vn.x, 
						p1.y + scaleFactor *getRadius()*vn.y);
				Point2D.Double interBackward = new Point2D.Double(p1.x - scaleFactor *getRadius()*vn.x, 
						p1.y - scaleFactor *getRadius()*vn.y);
		...
		if (getBases().size()>=2)
		{
			...
			Point2D.Double p2 = realCoords[j-1];
			Point2D.Double p3 = realCenters[j];
			isDirect = ControleurClicMovement.testDirectionality(p1, p2, p3);
		}
