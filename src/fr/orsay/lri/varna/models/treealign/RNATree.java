				...
				if (indexOfAssociatedBase >= 0) {
					if (indexOfAssociatedBase > i) {
						...
						if (base instanceof ModeleBaseNucleotide) {
							childValue.setLeftNucleotide(((ModeleBaseNucleotide) base).get_c());
							childValue.setRightNucleotide(((ModeleBaseNucleotide) rna.getBaseAt(indexOfAssociatedBase)).get_c());
						}
						i++;
					...
					ArrayList<ModeleStyleBP> auxBasePairs = rna.getAuxBPs(i);
					for (ModeleStyleBP auxBasePair: auxBasePairs) {
